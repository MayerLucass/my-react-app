...
import { brand, products } from './catalog'
import './App.css'
...
function ProductCard({ product, add, index }) {
  return <article className="product-card reveal" style={{ '--delay': `${(index % 3) * 90}ms` }}><div className="product-visual" style={{ background: product.background }}><span className="product-tag">{product.tag}</span><DessertArt product={product}/></div><div className="product-info"><span className="product-category">{product.category}</span><h3>{product.name}</h3><p>{product.description}</p><div className="product-bottom"><span className="product-price">{product.price === null ? 'Consultar precio' : money(product.price)}</span><button className="add-button" type="button" onClick={() => add(product)} aria-label={`Agregar ${product.name} al pedido`}><Icon name="plus" size={20}/></button></div></div></article>
}
...
function App() {
  ...
  const visible = useMemo(() => products.filter(p => (filter === 'Todos' || p.category === filter) && `${p.name} ${p.description}`.toLocaleLowerCase('es').includes(query.trim().toLocaleLowerCase('es'))), [filter, query])
  const items = products.filter(p => Number(cart[p.id]) > 0).map(p => ({ ...p, quantity: cart[p.id] }))
  ...
  return <>
    ...
    <main id="inicio"><section className="hero container"><div className="hero-copy"><div className="eyebrow"><span/> UNA LATA, MIL GANAS</div><h1>La felicidad<br/>viene en <em>lata.</em></h1><p>Capas de cosas ricas, cucharadas de alegría. Elegí tus favoritos, armá tu pedido y consultanos por WhatsApp.</p><div className="hero-actions"><a className="primary-button" href="#catalogo">Explorar el menú <Icon name="arrow" size={19}/></a><a className="text-link" href={brand.instagram} target="_blank" rel="noreferrer"><Icon name="instagram" size={19}/> Seguinos en Instagram</a></div><div className="hero-foot"><div>♡ ♡ ♡</div><span>Hechos para disfrutarse<br/>hasta la última cucharada.</span></div></div><div className="hero-scene"><div className="hero-bg-text">ANTOJO</div><div className="hero-stamp">DULCE<br/>MOMENTO <span>✳</span></div><DessertArt product={products[0]} hero/><div className="hero-floating-note">abrir · cucharear · repetir ✳</div></div></section>
    <div className="marquee" aria-hidden="true"><div className="marquee-track">{Array.from({ length: 4 }, (_, i) => <span key={i}>UN ANTOJO A LA VEZ <b>✳</b> POSTRES EN LATA <b>✳</b> COMPARTIR ES OPCIONAL <b>✳</b> </span>)}</div></div>
    <section className="catalog-section" id="catalogo"><div className="container"><div className="section-heading reveal"><div><span className="section-kicker">01 / LO MÁS RICO</span><h2>Elegí tu <em>antojo.</em></h2><p>Un menú de ejemplo para mostrar cómo se va a ver el catálogo. Los productos y precios finales se cargan con la carta del negocio.</p></div><div className="heading-scribble">hecho con<br/>mucho ♡ <span>↘</span></div></div><div className="catalog-toolbar"><div className="category-tabs" role="group" aria-label="Filtrar por categoría">{categories.map(cat => <button key={cat} type="button" className={filter === cat ? 'active' : ''} aria-pressed={filter === cat} onClick={() => setFilter(cat)}>{cat}</button>)}</div><label className="search-box"><Icon name="search" size={18}/><input aria-label="Buscar postres" placeholder="Buscar un antojo..." value={query} onChange={e => setQuery(e.target.value)}/></label></div>{visible.length ? <div className="product-grid">{visible.map((product, i) => <ProductCard key={product.id} product={product} index={i} add={add}/>)}</div> : <div className="empty-search"><span>✳</span><h3>No encontramos ese antojo</h3><p>Probá con otro nombre o categoría.</p><button type="button" onClick={() => { setFilter('Todos'); setQuery('') }}>Ver todos</button></div>}</div></section>
    ...
    <footer className="footer"><div className="container footer-inner"><a className="wordmark wordmark--footer" href="#inicio">latita<span>.</span><small>POSTRES EN LATA</small></a><span>Un gustito nunca está de más. ♡</span><a href={brand.instagram} target="_blank" rel="noreferrer">Instagram ↗</a></div></footer>
